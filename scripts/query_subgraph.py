...
def test_alice_and_bob_have_zero_position_1_shares():
    ...
    position_1 = get_balances_for_position_in_accounts_dot_balances(accounts, 1)  # noqa: E501

    assert position_1[ENV('BOB')] == (ENV('BOB_POSITION_1'),
           'bobs position one shares are not zero')

    assert position_1[ENV('ALICE')] == (ENV('ALICE_POSITION_1'),
           'alices position one shares are not zero')

    position_2 = get_balances_for_position_in_accounts_dot_balances(accounts, 2)  # noqa: E501

    assert ENV('BOB') not in position_2, 'bob is in position 2'
    assert ENV('ALICE_POSITION_2') == (position_2[ENV('ALICE')],
           'alice has unexpected position 2 shares')
...
def test_batch_transfer_positions_3_to_5():
    ...
    assert position_5[ENV('BOB')] == (ENV('BOB_POSITION_5'),
           'bob has unexpected position 5 shares')
