...
def test_enter_oi_error_collateral_minimum_not_met():
    '''
    Tests that the OverlayV1Market enterOI function fails when the provided
    collateral amount is less than the MIN_COLLAT plus the impact fee and the
    fee.
    Checks:
...
def test_update():
    ...
    - NewPricePoint Event emitted in OverlayV1PricePoint contract function:
      setPricePointNext
    - FundingPaid Event emitted in OverlayV1PricePoint contract function:
      payFunding
    Suggestion: Some event should be fired when making calls to outside
    contracts
    '''
    pass
